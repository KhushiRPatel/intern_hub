...
function ResetPasswordForm() {
  ...
  if (state === 'loading') {
    return (
      <div className="flex flex-col items-center gap-4 py-8">
        <div className="w-10 h-10 rounded-full border-2 border-t-transparent border-primary-500 animate-spin" />
        <p className="text-slate-500 dark:text-slate-400 text-sm">Validating your link…</p>
      </div>
    );
  }
  ...
  if (state === 'invalid') {
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm">{tokenError}</p>
  ...
  if (state === 'success') {
    return (
      <div className="text-center space-y-4">
        <div className="w-16 h-16 mx-auto bg-primary-600 rounded-2xl flex items-center justify-center shadow-lg shadow-primary-900/30">
  ...
  return (
    <>
      <div className="text-center mb-8">
        <div className="w-14 h-14 mx-auto bg-primary-600 rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-primary-900/30">
          <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
          </svg>
        </div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Set Your Password</h2>
        {email && (
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Setting password for <span className="font-semibold text-primary-600 dark:text-primary-400">{email}</span>
          </p>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Input
          ...
          rightAddon={
            <button type="button" onClick={() => setShowPw(v => !v)} className="text-slate-400 hover:text-primary-500 transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                {showPw
        ...
        <div className="w-full max-w-sm animate-fade-in">
          ...
          <Suspense fallback={
            <div className="flex justify-center py-8">
              <div className="w-8 h-8 rounded-full border-2 border-t-transparent border-primary-500 animate-spin" />
            </div>
          }>
            <ResetPasswordForm />
          </Suspense>
